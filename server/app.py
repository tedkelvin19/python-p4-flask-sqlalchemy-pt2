...
def pet_by_id(id):
    ...
    response_body = f'''
        ...
        <h2>Pet Owner: {pet.owner.name}</h2>
'''
...
def owner_by_id(id):
    ...
    if not owner:
        response_body = f'''
            <h1>No owner found with id {id}</h1>
        '''
        response = make_response(response_body, 404)
        return response
    ...
    if not pets:
        response_body += f'<h2>Has no pets found for at this time.{owner.name}</h2>'
    else:
        for pet in pets:
